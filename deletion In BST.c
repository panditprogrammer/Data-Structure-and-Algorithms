...
void insert(struct Node *root,int element)
{
    ...
    {
        previous_node = root;
        if(element == root->data)   // insertion not possible
        {
            printf("Element already exist!\n");
            return;
        }
        else if(element < root->data)  // if element is less than root data update root
            root = root->left;
        else
            root = root->right;   // if element is greater than root data update root
    }
...
struct Node* deleteInBST(struct Node *root,int element)
{
    ...
    {
        inorderPre = inOrderPrecessor(root);
        root->data = inorderPre->data;
        root->left = deleteInBST(root->left,inorderPre->data);
    }
    return root;
}
